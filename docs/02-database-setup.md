...
| 명령 | 컨테이너 | 볼륨(데이터) | 이미지 |
|---|---|---|---|
| `docker compose stop` | **중지** (남아있음) | 보존 | 보존 |
| `docker compose down` | **삭제** | 보존 | 보존 |
| `docker compose down -v` | 삭제 | **삭제** 💥 | 보존 |

**데이터 관점에서는 `stop` 과 `down` 이 동일하다.** 진짜 조심할 건 `-v` 하나뿐.

**이 프로젝트는 `down` 을 쓴다.** 이유 세 가지:

1. **설정 변경이 항상 반영된다** ⭐
   `stop` → `start` 는 기존 컨테이너를 그대로 켜므로 **`docker-compose.yml` 을 고쳐도 반영되지 않는다.**
   학습 단계에서는 yml 을 자주 고치므로 "분명 고쳤는데 왜 안 바뀌지?" 함정에 빠지기 쉽다.
2. **컨테이너 목록이 안 더러워진다**
   `stop` 만 반복하면 종료된 컨테이너가 계속 쌓인다.
   (실제로 이 PC 에는 16개월 전 nginx 컨테이너가 아직 목록에 남아 있었다)
3. **yml 이 동작하는지 매번 검증된다**
   `up -d` 때마다 "파일만으로 환경이 재현되는가" 가 확인된다.

`stop` 이 나은 경우: 하루에 수십 번 껐다 켤 때 (재시작이 ~1초 vs ~3초)

> **🐄 가축 vs 애완동물**
>
> | | 애완동물(Pet) | 가축(Cattle) |
> |---|---|---|
> | 아프면 | 접속해서 고침 | **버리고 새로 만듦** |
>
> **컨테이너는 가축으로 다뤄야 한다.** `down` 을 습관화하면
> "컨테이너 안에 뭘 설치해뒀는데" 라는 걱정이 사라지고,
> **필요한 건 전부 yml 에 적게 된다.**
> "손으로 한 건 기억에 남고, 파일로 적은 건 git 에 남는다" 와 같은 뿌리.

---

### 📋 컨테이너에서 설정할 수 있는 것 / 없는 것

`docker-compose.yml` 의 설정은 **담당이 다른 두 층**이 한 파일에 섞여 있다.

#### ① Docker 가 관리하는 것 — 컨테이너 "껍데기"

컨테이너 바깥 세계와의 접점.

| 설정 | 하는 일 | 현재 |
|---|---|---|
| `image` | 어떤 프로그램을 쓸지 | `mysql:8.0` ✅ |
| `ports` | **포트 포워딩** | `127.0.0.1:3306:3306` ✅ |
| `volumes` | 저장소 연결 | `snapcal-mysql-data` ✅ |
| `restart` | 죽으면 자동 재시작 | `unless-stopped` ✅ |
| `healthcheck` | 살아있는지 주기 확인 | `mysqladmin ping` ✅ |
| `container_name` | 이름 지정 | `snapcal-mysql` ✅ |
| `name` (최상위) | compose 프로젝트명 고정 | `snapcal` ✅ |
| `networks` | 컨테이너끼리 통신 범위 | 미사용 (기본값) |
| `depends_on` | 기동 순서 | 미사용 |
| `mem_limit` / `cpus` | 자원 제한 | 미사용 |
| `user` | 어떤 OS 계정으로 실행 | 미사용 |

#### ② 컨테이너 안 프로그램에 전달하는 것

Docker 가 직접 하는 게 아니라 **MySQL 에게 넘겨주는** 값.

| 설정 | 하는 일 |
|---|---|
| `environment` | MySQL 이 읽어서 계정·DB 생성 |
| `command` | mysqld 실행 옵션 (`--character-set-server=utf8mb4`) |
| `entrypoint` | 시작 스크립트 자체 교체 (거의 안 씀) |

현재 사용 중: `MYSQL_ROOT_PASSWORD`, `MYSQL_DATABASE`, `TZ`

미사용:
```yaml
MYSQL_USER: snapcal_app        # 전용 계정 생성
MYSQL_PASSWORD: xxxx
MYSQL_ROOT_HOST: localhost     # root 접속 허용 범위 제한
```
...
| 하고 싶은 것 | 컨테이너 설정으로? |
|---|---|
| 계정 **하나 만들기** | ✅ `MYSQL_USER` — 단 **최초 1회만** |
| **권한 세부 조정** (`GRANT SELECT ON ...`) | ❌ **SQL 로만** |
| 계정 **추가** (두 번째 이후) | ❌ SQL 로만 |
| 비밀번호 **변경** | ❌ SQL(`ALTER USER`) 로만 |
| 테이블 만들기 | ❌ 마이그레이션으로 |

**왜냐면 `environment` 는 컨테이너가 처음 태어날 때 한 번만 읽힌다.**
그 뒤로는 무시된다. (비밀번호 변경에 `down -v` 가 필요했던 이유가 정확히 이것)

> **경계선**
> 컨테이너 설정 = **"MySQL 서버를 어떻게 띄울까"**
> SQL = **"떠 있는 MySQL 안에서 뭘 할까"**

#### 아직 안 썼지만 유용한 것

**초기화 스크립트** — 계정/권한을 파일로 남기는 방법
```yaml
volumes:
  - ./init:/docker-entrypoint-initdb.d
```
이 폴더의 `.sql` 파일은 **컨테이너가 처음 만들어질 때 자동 실행**된다.
"손으로 한 것"을 "파일로 적은 것"으로 바꿀 수 있다.

**메모리 제한** — MySQL 은 약 450MB 를 쓴다
```yaml
mem_limit: 512m
```

**네트워크** — Redis 등을 추가할 때 필요.
같은 네트워크의 컨테이너는 **포트 매핑과 무관하게 서로 접근 가능**하다.
